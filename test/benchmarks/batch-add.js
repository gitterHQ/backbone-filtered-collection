...
Benchmark.add('Old filtering collections by batch adding models', function() {
  oldFilteredCollection.setFilter(filter);
  collection.add(_.range(100).map(function(i) {
    return { id: i };
  }));
  assert.equal(50, oldFilteredCollection.length);
});

Benchmark.add('New filtering collections by batch adding models', function() {
